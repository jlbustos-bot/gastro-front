...
import { grupo2prodService, Grupo2Prod } from '../services/grupo2prodService';
import './Grupo1Prod.css';

const createEmptyForm = (): Grupo2Prod => ({
  nombre: '',
  activo: true,
});

const Grupo2ProdPage = () => {
  const [items, setItems] = useState<Grupo2Prod[]>([]);
  const [loading, setLoading] = useState(true);
  const [showForm, setShowForm] = useState(false);
  const [editingId, setEditingId] = useState<number | null>(null);
  const [formData, setFormData] = useState<Grupo2Prod>(() => createEmptyForm());
  const [isImporting, setIsImporting] = useState(false);
  const fileInputRef = useRef<HTMLInputElement | null>(null);

  const fetchItems = async () => {
    try {
      const data = await grupo2prodService.getAll();
      setItems(data);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        const updated = await grupo2prodService.update(editingId, payload);
        setItems((prev) => prev.map((item) => (item.id === updated.id ? updated : item)));
      } else {
        const created = await grupo2prodService.create(payload);
  ...
  const handleEdit = (item: Grupo2Prod) => {
  ...
  const handleDelete = async (id?: number) => {
    ...
    try {
      await grupo2prodService.delete(id);
  ...
  const handleDownloadTemplate = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Grupo2Prod');
    XLSX.writeFile(workbook, 'plantilla_grupo2prod.xlsx');
  };
  ...
  const handleExportExcel = () => {
    ...
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Grupo2Prod');
    XLSX.writeFile(workbook, 'grupo2prod.xlsx');
  };
  ...
  const handleExportPdf = () => {
    ...
    const pdf = new jsPDF();
    pdf.setFontSize(14);
    pdf.text('Grupo 2 Productos', 14, 15);
  ...
  const handleImportExcel = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const row of normalizedRows) {
        await grupo2prodService.create({ nombre: row.nombre, activo: row.activo });
      }
